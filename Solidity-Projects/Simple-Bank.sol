...
contract Simplebankinterface is bankrequirements {
    ...
    function withdraw(uint amount) public returns(string memory){               //checks if the user has enough funds to withdraw 
            require(amount <= deposits[msg.sender], "insufficient funds");      //and then sends their funds after subtracting 
            deposits[msg.sender] -= amount;                                     //from their total deposits and updating totalDeposits.
